...
openssl req \
      ...
      -new \
      -newkey rsa:2048 \
      -keyout cert/a-ext-key.pem \
      -out csr/a-ext-csr.pem \
      -config cnf/a.cnf
...
# -config : Additional configuration options
# -extfile: File containing certificate extensions
openssl ca \
      -in csr/a-ext-csr.pem \
      -out cert/a-ext-cert.pem \
      -days 3650 \
      -notext \
      -batch \
      -config cnf/openssl-req.cnf \
      -extfile cnf/extensions.cnf
